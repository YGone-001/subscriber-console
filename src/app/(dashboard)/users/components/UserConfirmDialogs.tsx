...
export function UserConfirmDialogs(props: UserConfirmDialogsProps) {
  ...
  return (
    <>
      {props.pendingDeleteUsername ? (
        <ConfirmActionPanel
          presentation="modal"
          title={t("users_delete_confirm", { username: props.pendingDeleteUsername })}
          message={t("users_delete_desc")}
          confirmLabel={t("delete")}
          cancelLabel={t("cancel")}
          isWorking={props.savingAction === `delete:${props.pendingDeleteUsername}`}
          confirmDisabled={props.confirmReason.trim().length < 3}
          onConfirm={props.executeDelete}
          onCancel={props.resetConfirmState}
        >
          <ConfirmDetails target={props.pendingDeleteUsername} approval={t("users_confirm_approval_none")} irreversible={t("users_confirm_irreversible_yes")} reason={props.confirmReason} setReason={props.setConfirmReason} />
        </ConfirmActionPanel>
      ) : null}
      ...
      {props.pendingBulkAction ? (
        <ConfirmActionPanel
          presentation="modal"
          tone={props.pendingBulkAction.action === "delete" || props.pendingBulkAction.action === "disable" ? "warning" : "info"}
        ...
        >
          <ConfirmDetails
            ...
            irreversible={props.pendingBulkAction.action === "delete" ? t("users_confirm_irreversible_yes") : t("users_confirm_irreversible_no")}
