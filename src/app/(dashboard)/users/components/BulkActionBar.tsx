import { Download, Shield, UserCheck, UserX, X } from "lucide-react";
...
type BulkActionBarProps = Pick<
  UsersTableProps,
  "selectedUsernames" | "mutableSelectedCount" | "requestBulkAction" | "bulkRole" | "setBulkRole" | "exportSelectedUsers" | "clearSelection" | "assignableRoles"
>;
  ...
  return (
    <div className={styles.bulkBar} role="region" aria-label={t("users_bulk_actions")}>
      ...
      <div className={styles.bulkActions}>
        {props.assignableRoles.length > 0 ? <>
        ...
        <label>
          ...
          <select className="form-input" value={props.bulkRole} onChange={(event) => props.setBulkRole(event.target.value as RoleKey)} disabled={disabled}>
            {props.assignableRoles.map((role) => <option key={role} value={role}>{t(`users_${role}`)}</option>)}
          </select>
        </label>
