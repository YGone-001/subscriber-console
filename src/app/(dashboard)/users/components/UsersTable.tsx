...
export function UsersTable(props: UsersTableProps) {
  ...
  useEffect(() => {
    ...
    const closeOnViewportChange = () => setOpenMenuUsername(null);
  ...
  const openMoreMenu = (username: string, trigger: HTMLButtonElement) => {
    ...
    setOpenMenuUsername((current) => current === username ? null : username);
  };
        ...
        <table className={styles.table}>
          ...
          <thead>
            <tr>
              ...
              <th className={styles.roleCol} data-column-priority="essential">{t("users_role")}</th>
              <th className={styles.statusCol} aria-sort={getAriaSort("status")} data-column-priority="essential">
              ...
              <th className={styles.lastLoginCol} aria-sort={getAriaSort("lastLoginAt")} data-column-priority="important">
          ...
          <tbody>
            {props.isLoading ? (
              ...
              <tr className={styles.tableStateRow}><td colSpan={6}><EmptyState icon={<UserCheck size={44} />} title={t("users_empty")} description={t("users_empty_desc")} action={props.canCreate ? <button type="button" className="btn btn-primary" onClick={props.openCreateDrawer}><Plus size={16} />{t("users_new")}</button> : undefined} /></td></tr>
              ...
              return (
                <tr key={item.username}>
                  ...
                  <td className={styles.roleCol} data-label={t("users_role")} data-column-priority="essential"><RoleBadge role={item.role} /></td>
                  <td className={styles.statusCol} data-label={t("users_status")} data-column-priority="essential"><StatusBadge status={item.status} locked={item.locked} /></td>
                  <td className={`${styles.dateCell} ${styles.lastLoginCol}`} data-label={t("users_last_login")} data-column-priority="important"><span>{formatLatestLoginTime(item.security?.lastLoginAt, item.lastLoginAt)}</span></td>
                  <td className={styles.actionsCol} data-label={t("users_actions")} data-column-priority="essential">
                    <div className={styles.rowActions}>
                      ...
                      <div className={styles.more}>
                        <button type="button" className="btn-icon" onClick={(event) => openMoreMenu(item.username, event.currentTarget)} aria-expanded={props.openMenuUsername === item.username} aria-haspopup="menu" title={t("users_more_actions")} aria-label={t("users_more_actions")}><MoreHorizontal size={17} /></button>
                        {props.openMenuUsername === item.username && menuPosition && typeof document !== "undefined" ? createPortal(
                          <div className={styles.moreMenu} ref={openMenuRef} role="menu" aria-label={t("users_more_actions")} style={{ position: "fixed", top: menuPosition.top, left: menuPosition.left, right: "auto" }}>
