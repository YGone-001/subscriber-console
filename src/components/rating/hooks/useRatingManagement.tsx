import { Database, MessageSquare, Mic2, ShieldCheck, Tag } from "lucide-react";
...
export function useRatingManagement(view: T.RatingManagementView) {
  ...
  const cancelPlanEdit = () => { setIsCreatingPlan(false); setPlanForm({ plan_id: "", name: "", description: "", status: "active" }); };
  return {
    t,
    canEditTemplates,
    plans,
    ...
    selectedPlan,
    isCreatingPlan, setIsCreatingPlan,
    planForm, setPlanForm,
    beginCreatePlan, handleCreatePlan, handleUpdatePlan, cancelPlanEdit,
    handleDeletePlan, handleMigratePlanSubscribers, formatPlanOperationAction,
    migrationTargetPlanId, setMigrationTargetPlanId,
    migrationTargetOptions,
    migrationResetBalances, setMigrationResetBalances,
    selectedPlanSubscribers,
    selectedPlanSubscriberTotal,
    planOperationSummary, planOperationHistory,
    isDisablingPlanWithSubscribers,
    ratings, enrichedRatings, visibleRatings, counts,
    filter, setFilter,
    query, setQuery,
    notice, setNotice,
    savingKey,
    ...
    newForm, setNewForm,
    validateRatingForm,
    handleCreate, handleUpdate, handleDelete, executeDelete,
    applyChargingType, rateTypes, CURRENCIES, formatCurrency,
    startEdit, serviceMeta,
