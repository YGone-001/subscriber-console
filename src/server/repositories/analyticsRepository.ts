...
export type AnalyticsMetrics = {
  ...
  top5: Array<{ imsi: string; balance: number; voiceBalance: number }>;
  timestamp: number;
};
...
export async function computeAnalyticsMetrics(): Promise<AnalyticsMetrics> {
  ...
  const cursor = balances.find({}, { projection: { imsi: 1, data_available: 1, voice_available: 1 } });
  let totalTraffic = 0;
  const plmnMap = new Map<string, number>();
  const policyCount = policy ? await balances.estimatedDocumentCount() : 0;
  const leaderboard: Array<{ imsi: string; balance: number; voiceBalance: number }> = [];
  ...
  for await (const balanceDoc of cursor) {
    ...
    if (balance > 0) {
      ...
      leaderboard.push({ imsi: balanceDoc.imsi, balance, voiceBalance });
    }
  }
