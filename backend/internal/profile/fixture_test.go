...
func TestRestoreFingerprintParity(t *testing.T) {
	...
	assertEqual(t, "sourceVersionHash", sourceVersionHash, "3f33fcbbdf86b5b6e7385bfdd2e410e94f6da04cf126dd1163234c9c76a0bf60")

	// Compute currentProfileHash
	currentProfileHash := fingerprint(RestoreCurrentProfile)
	assertEqual(t, "currentProfileHash", currentProfileHash, "270d5f1afbe0d64e7ec3ed6df3f7cf6d56eb9867111987cc7be878751de2ba01")

	// Use deterministic clock for cross-runtime parity
	deterministicClock := time.Date(2024, 6, 1, 10, 0, 0, 0, time.UTC)

	// Build effective restored profile using actual production function
	restored := buildEffectiveRestoredProfile(RestoreCurrentProfile, RestoreVersionDoc, "fixture_restore", "admin", deterministicClock)

	effectiveRestoredHash := fingerprint(restored)
	assertEqual(t, "effectiveRestoredHash", effectiveRestoredHash, "04d4d79798be9f7adb5242423a63cf4dbb8324a05beeea193e62dec9b87ba2cb")
	...
	assertEqual(t, "operationFingerprint", opFp, "b02a280c12c5f5e4f89f6199d62aa45f5406d7cd32104075cde61aafac1fd970")
}
