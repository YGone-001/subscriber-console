...
export default function SubscriberBatchUpdateModal({ isOpen, selectedImsis, onClose, onSuccess }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.code || body.error || "Unable to update subscribers.");
      onSuccess(body); onClose();
    } catch (submitError) { setError(submitError instanceof Error ? submitError.message : "Unable to update subscribers."); }
  ...
  return <Dialog open={isOpen} onClose={() => { if (!isSaving) onClose(); }} overlayClassName="modal-overlay" className="modal-content animate-modal-enter bu-modal-content" labelledBy="subscriber-batch-update-title" initialFocusRef={cancelRef} closeOnOverlay={!isSaving}>
    <div className="workflow-header bu-header"><div><h2 id="subscriber-batch-update-title" className="bu-title"><Settings2 size={18} /> 批量修改订阅用户</h2><p>Authorized changes execute immediately after server-side validation.</p></div><button type="button" className="btn-icon" onClick={onClose} aria-label="Close" disabled={isSaving}><X size={22} /></button></div>
    <div className="bu-body">
      <div className="bu-warning"><AlertTriangle size={17} /><span>提交前请确认目标用户与字段；通过校验后将立即执行。</span></div>
      ...
      {error ? <OperationNotice presentation="modal" tone="danger" title="无法执行批量修改" message={error} onClose={() => setError(null)} /> : null}
    </div>
    <div className="workflow-footer bu-footer"><span>服务器将重新验证权限、业务规则与并发前置条件。</span><div className="workflow-footer-actions"><button ref={cancelRef} type="button" className="btn btn-outline" onClick={onClose} disabled={isSaving}>取消</button><button type="button" className="btn btn-primary" onClick={() => void submit()} disabled={isSaving || selectedImsis.length === 0}><Save size={16} />{isSaving ? "正在执行…" : "立即执行"}</button></div></div>
  </Dialog>;
}
