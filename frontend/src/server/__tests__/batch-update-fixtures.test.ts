...
import {
  ...
  COMBINED_FIXTURE,
  MULTI_IMSI_FIXTURE,
} from './batch-update-fixtures';

// Section Y: Cross-runtime frozen integrity tests
...
describe('Batch Update Cross-Language Fixtures', () => {
  const FIXTURES: Record<string, { frozen: any; expectedLeafKeys: string[]; expectedPatch: Record<string, unknown> }> = {
    accessRestriction: {
  ...
  for (const [fixtureName, { frozen, expectedLeafKeys, expectedPatch }] of Object.entries(FIXTURES)) {
    describe(fixtureName, () => {
      it('frozen payload has correct targetCount', () => {
        assert.strictEqual(frozen.targetCount, frozen.targets.length);
      });

      it('frozen payload has sorted fieldNames', () => {
        const actual = (frozen.fieldNames as string[]).slice().sort();
        assert.deepStrictEqual(frozen.fieldNames, actual);
      });

      it('frozen payload has sorted targets by IMSI', () => {
        const imsis = frozen.targets.map((t: any) => t.imsi);
        const sorted = [...imsis].sort();
        assert.deepStrictEqual(imsis, sorted);
      });

      it('frozen payload snapshotBytes is plausible', () => {
      ...
      it('frozen payload operationFingerprint is non-empty', () => {
        assert.ok(frozen.operationFingerprint.length > 0);
      });

      it('before/after key sets exactly match expected leaf keys', () => {
      ...
      it('preconditionHash matches fingerprint of before', () => {
        for (const target of frozen.targets) {
      ...
      it('after values match expected from patch', () => {
        for (const target of frozen.targets) {
          if ('accessRestrictionData' in expectedPatch) {
            assert.strictEqual(target.after.access_restriction_data, (expectedPatch as any).accessRestrictionData);
          }
          const ambr = (expectedPatch as any).ambr;
          if (ambr?.downlink) {
      ...
      it('IMSIs are exactly 15 ASCII digits', () => {
        for (const target of frozen.targets) {
