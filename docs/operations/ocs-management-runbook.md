...
Go Backend Service (backend/cmd/server)
  ...
  - Enforces Maker-Checker governance policy
  - Enforces atomic CAS preconditions
  - Writes audit trail synchronously (WriteStrict)
           │
           ▼
MongoDB Infrastructure
  ...
  └── Database: xcloud_ops
        ├── app_approvals          (Maker-checker governance tickets)
        └── app_audit_logs         (Immutable operational audit logs)
```
...
1. **Create Plan**:
   ...
   - If Operator: Pending approval request is created. Super Admin must approve before plan is committed.
2. **Update Plan**:
   - Submit `PUT /api/tariff-plans/{planId}` with updated quota or validity parameters.
   - Triggers version increment `version += 1`.
...
5. **View Operations History**:
   - Navigate to `/ocs/tariffs/[planId]` to inspect the changelog and audit events recorded in `xcloud_ops.app_audit_logs`.
...
3. Specify:
   ...
   - `reason` (mandatory audit trail explanation)
4. Execution:
   - **Direct (Super Admin)**: Applied immediately to `xcloud.ocs_balances`.
   - **Approval (Operator)**: Submits approval request ticket. Balance is updated only when second administrator approves.
...
## 6. Governance & Maker-Checker Workflows

All non-root administrative writes require maker-checker approval.

### 6.1 Approval Flow Diagram
```text
Maker (Operator)           Console / Go Backend                  Checker (Super Admin)
     │                              │                                      │
     │── 1. Submit mutation ───────>│                                      │
     │   (e.g. adjust balance)      │                                      │
     │                              │── 2. Create approval ticket ────────>│
     │                              │      Status: PENDING                 │
     │                              │      Records: payload + CAS version  │
     │                              │                                      │
     │                              │<── 3. Approve ticket ────────────────│
     │                              │                                      │
     │                              │── 4. Verify CAS version              │
     │                              │   5. Apply database mutation         │
     │                              │   6. Mark ticket APPROVED            │
     │                              │   7. Synchronous audit log entry     │
     ▼                              ▼                                      ▼
```

### 6.2 Key Security Controls
1. **Replay Protection**: An approval ticket cannot be executed more than once. The execution step atomically transitions status from `pending` to `approved` under a single CAS filter.
2. **Stale Execution Prevention**: During execution, the stored `expected_version` is validated against the live document version in MongoDB. If the balance or plan was modified after the ticket was created, the execution fails with HTTP 409 `CAS_CONFLICT`.
3. **Separation of Duties**: A user cannot approve their own request ticket (`MAKER_CANNOT_BE_CHECKER`).
...
### 7.1 Health Checks
- Go Backend: `GET http://127.0.0.1:18888/api/health` or `/healthz`
  - Expected: HTTP 200 `{"status":"ok","version":"..."}`
- Ingress Proxy: `GET http://127.0.0.1:13333/api/health`
...
| `APPROVAL_REQUIRED` | 202 / 200 | Operator submitted governed action | Check Approvals dashboard (`/governance/approvals`) for checker signoff. |
| `MAKER_CANNOT_BE_CHECKER`| 403 | Creator attempted self-approval | Another administrator with `super_admin` or `ops_admin` role must approve. |
| `GO_BACKEND_UNREACHABLE` | 502 | Go process (:18888) is down | Check Go service systemd/supervisor process and restart if necessary. |
...
### 7.4 Audit Log Inquiries
All operations are logged synchronously to `xcloud_ops.app_audit_logs`.  
Querying audit logs for a specific subscriber:
```javascript
...
All future development must focus on Phase 6 (Authentication & User Management).
