...
| Subscriber bulk delete | POST | `/api/subscribers/bulk-delete` | **Go** | Next.js | Go | NEVER | Go implementation with frozen v2, per-target CAS, ACTUALLY_ROUTED=1 |
...
| Profile delete | DELETE | `/api/profiles/:name` | **Go** | **Go** | Go | NEVER | DIRECT_GOVERNED, ACTUALLY_ROUTED=1, cutover_verified |
| Profile restore | POST | `/api/profiles/:name/versions/:versionId/restore` | **Go** | **Go** | Go | NEVER | Go handler with frozen v2, CAS, approval, ACTUALLY_ROUTED=1, cutover_verified |
...
| OCS balance adjust | POST | `/api/ocs/balances/:imsi/adjust` | **Go** | **Go** | Go | NEVER | Phase 5.4-B controlled production cutover; ACTUALLY_ROUTED=1; cutover_verified |
| OCS balance reset | POST | `/api/ocs/balances/:imsi/reset` | **Go** | **Go** | Go | NEVER | permanently disabled compatibility endpoint; BALANCE_RESET_DISABLED; ACTUALLY_ROUTED=1; cutover_verified |
